...
export default function AdminPanel() {
  ...
  const handleAddGame = async (game: any) => {
    ...
    try {
      // 🌟 اینجا کل دیتای دریافتی را بدون جا انداختن فیلدی به بک‌اند می‌فرستیم
      const res = await fetch('/api-store', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          platforms: game.platforms, // شامل سیستم مورد نیاز PC
          genres: game.genres,
          playtime: game.playtime,
          esrb_rating: game.esrb_rating,
          developers: game.developers || [], // ➕ اضافه شد
          publishers: game.publishers || [], // ➕ اضافه شد
          stores: game.stores || [],         // ➕ اضافه شد
          tags: game.tags || [],             // ➕ اضافه شد
          clip: game.clip || null            // ➕ تریلر فیلم بازی
        }),
      });
      ...
      setMessage({ text: `بازی "${game.name}" با موفقیت و دیتای کامل به دیتابیس اضافه شد.`, isError: false });
      fetchMyGames();
    } catch (err: any) {
      setMessage({ text: err.message, isError: true });
    }
      ...
      <div className="min-h-screen bg-slate-900 flex flex-col justify-center items-center p-4" dir="rtl">
        <div className="bg-slate-800 p-8 rounded-2xl shadow-xl w-full max-w-md border border-slate-700">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button type="submit" className="w-full py-3 bg-purple-600 hover:bg-purple-700 text-white font-bold rounded-xl transition">
              ورود به پنل
            </button>
          </form>
        </div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <header className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-8 border-b border-slate-800 pb-4">
          ...
          <div className="flex items-center gap-3">
            <Link href="/" className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-xl transition text-sm font-medium">
              🌐 مشاهده سایت اصلی
            </Link>
            <button onClick={() => setIsLoggedIn(false)} className="px-4 py-2 bg-slate-800 hover:bg-red-900/40 text-red-400 border border-slate-700 rounded-xl transition text-sm font-medium">
              خروج از پنل
            </button>
          </div>
        </header>
        ...
        <section className="bg-slate-800 p-6 rounded-2xl border border-slate-700 mb-8 shadow-xl">
          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-4">
            ...
            />
            <button type="submit" disabled={loading} className="px-8 py-4 bg-purple-600 hover:bg-purple-700 text-white font-bold rounded-xl transition">
              {loading ? 'در حال جستجو...' : 'جستجوی بازی'}
            </button>
          </form>
          {message.text && (
            <div className={`mt-4 p-4 rounded-xl text-center font-medium border ${message.isError ? 'bg-red-900/30 text-red-400 border-red-500/50' : 'bg-green-900/30 text-green-400 border-green-500/50'}`}>
              {message.text}
            </div>
          )}
        </section>
        ...
        {loading && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 animate-pulse">
            {[1, 2, 3, 4].map((n) => (
              <div key={n} className="bg-slate-800/50 border border-slate-700 rounded-2xl h-64 p-4 flex flex-col justify-between">
                  ...
                  <div key={game.id} className="bg-slate-800 border border-slate-700 rounded-2xl overflow-hidden shadow-lg flex flex-col justify-between group hover:border-slate-500 transition duration-300">
                    <div className="relative aspect-video w-full bg-slate-900 overflow-hidden">
                      {game.background_image ? (
                        <img src={getOptimizedImage(game.background_image)} alt={game.name} className="object-cover w-full h-full" loading="lazy" />
                      ) : (
                    ...
                    <div className="p-4 flex-1 flex flex-col justify-between">
                      <div className="mb-4">
                        <h3 className="font-bold text-white text-base line-clamp-1 mb-1">{game.name}</h3>
                        <p className="text-xs text-slate-400">انتشار: {game.released || 'نامشخص'}</p>
                      </div>
                      {isAlreadyAdded ? (
                        <button onClick={() => handleDeleteGame(game.id, game.name)} className="w-full py-2.5 bg-red-600/20 hover:bg-red-600 text-red-400 hover:text-white font-semibold rounded-xl border border-red-500/40 transition text-sm">
                          ❌ حذف از سایت
                        </button>
                      ) : (
                        <button onClick={() => handleAddGame(game)} className="w-full py-2.5 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-xl transition text-sm shadow-lg">
                          ➕ اضافه به سایت
                        </button>
                      )}
                    </div>
